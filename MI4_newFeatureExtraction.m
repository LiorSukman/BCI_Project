...
num4test = floor(0.25*trials);                                  % define how many test trials after feature extraction
...
% new bands
bands{1} = [0,5];         % noise
bands{2} = [5,8];         % noise+signal
bands{3} = [8,18];        % gap
bands{4} = [18,20];       % fading gap
bands{5} = [20,25]; 
bands{6} = [25,40]; 
...
% new times
times{1} = (0.64*Fs : 0.8*Fs);
times{2} = (1.28*Fs : 1.6*Fs);
times{3} = (2.08*Fs : 2.24*Fs);
times{4} = (2.88*Fs : 3.04*Fs);
times{5} = (3.52*Fs : 3.68*Fs);
times{6} = (4.32*Fs : 4.48*Fs);
...
for trial = 1:trials                                % run over all the trials
    ...
    for channel = 1:numChans                        % run over all the electrodes (channels)
        ...
        for feature = 1:numSpectralFeatures                 % run over all spectral band power features from the section above
            ...
            MIFeaturesLabel(trial,channel,n) = bandpower(squeeze(MIData(trial,channel,floor(times{feature}))),Fs,bands{feature});
            n = n+1; 
        end
